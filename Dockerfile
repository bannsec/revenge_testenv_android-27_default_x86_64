...
RUN apt update && apt dist-upgrade -y && \
    ...
    echo y | /opt/tools/bin/sdkmanager --sdk_root=${ANDROID_HOME} "build-tools;28.0.3" >/dev/null && \
    echo y | /opt/tools/bin/sdkmanager --sdk_root=${ANDROID_HOME} "platforms;android-27" >/dev/null && \
    ...
    echo y | /opt/tools/bin/sdkmanager --sdk_root=${ANDROID_HOME} "system-images;android-27;default;x86_64" >/dev/null && \
    echo no | /opt/tools/bin/avdmanager create avd --force --name test --package 'system-images;android-27;default;x86_64' && \
